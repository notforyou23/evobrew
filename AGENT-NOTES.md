...
- Main files touched: `public/index.html`, `public/js/ui-shell.js`, `public/css/ui-shell.css`, `public/css/ui-responsive.css`, `public/js/terminal.js`.
- Verified live in browser: working folder selection, workspace create/discard, brain attach, working-folder ↔ brain-folder switching, terminal open/close.

### Runtime Context Fixes

- Fixed missing active-folder source for workspace creation by adding `getActiveFolderPath()` in `public/index.html`.
- Added explicit runtime refresh dispatches for workspace activation/reset, folder switching, brain load, edit queue panel changes, and terminal dock open/close.
- Fixed the brain-folder/workspace mismatch so browsing a connected brain folder does not falsely show repo workspace availability unless a real active workspace exists.

### Known Notes

- Browser pass still shows a non-blocking `404` for `favicon.ico`.
- Brain load has a backend endpoint, but there is no matching frontend/browser unload flow wired in this pass.
- The runtime context component is still largely frontend-derived from existing DOM/global state, so label-format changes elsewhere could affect it.

### Suggested Next Step

- If continuing this line of work, prefer adding more explicit `window.refreshRuntimeContext?.()` or `evobrew:runtime-context-refresh` calls at the source of truth instead of expanding DOM observers.
