...
class PGSEngine {
  ...
  async execute(query, options = {}) {
    ...
    switch (mode) {
      ...
      default: // 'full'
        partitionsToSweep = pgsFullSweep ? partitions : allRoutedPartitions.slice(0, config.maxSweepPartitions);
    }
    ...
    // Sweeps use Sonnet (fast/cheap) — only final synthesis uses user-selected model
    const sweepModel = 'claude-sonnet-4-6';
    if (model !== sweepModel) {
