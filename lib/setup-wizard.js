 ...
 */
async function runAnthropicOAuthFlow(spinner) {
  console.log('');
  ...
  info(`If browser doesn't open, copy this URL:`);
  console.log(`\n${authUrl}\n`);
  info('After authorizing, you\'ll see a code. The URL will look like:');
  info('https://console.anthropic.com/oauth/code/callback?code=xxx#yyy');
  console.log('');
  
  const input = await questionSecret('Paste the full URL or just code#state:');
  ...
  if (trimmed.startsWith('http')) {
    ...
    state = url.hash.substring(1); // Remove #
  } else if (trimmed.includes('#')) {
    const parts = trimmed.split('#');
    code = parts[0];
    state = parts[1];
  } else {
    throw new Error('Invalid format. Need full URL or code#state');
  }
  
  if (!code || !state) {
    throw new Error('Could not extract code and state');
  }
  ...
  for (const provider of providers) {
    ...
    if (provider === 'anthropic') {
      ...
      if (authMethod === 'oauth') {
        ...
        const spin = spinner('Opening browser...');
        spin.start();

        try {
          await runAnthropicOAuthFlow(spin);
          spin.stop('success');
