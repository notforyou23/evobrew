...
const { getAuthorizationUrl, exchangeCodeForTokens, storeToken } = require('../server/services/anthropic-oauth');
...
// Anthropic OAuth (CLI-style local callback)
const ANTHROPIC_OAUTH_CALLBACK_PORT = 1455;
const ANTHROPIC_OAUTH_CALLBACK_PATH = '/auth/callback';
...
/**
 * Start local callback server for OAuth return.
 */
function startAnthropicOAuthCallbackServer(expectedState) {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      const url = new URL(req.url, `http://localhost:${ANTHROPIC_OAUTH_CALLBACK_PORT}`);

      if (url.pathname !== ANTHROPIC_OAUTH_CALLBACK_PATH) {
        res.statusCode = 404;
        res.end('Not found');
        return;
      }
      ...
      res.end(`<!DOCTYPE html><html><body><h1>✅ Authentication Successful</h1><p>You can return to your terminal now.</p></body></html>`);

      server.close();
      resolve(code);
    });

    server.listen(ANTHROPIC_OAUTH_CALLBACK_PORT, '127.0.0.1', () => {});

    server.on('error', (err) => {
      if (err.code === 'EADDRINUSE') {
        reject(new Error(`Port ${ANTHROPIC_OAUTH_CALLBACK_PORT} already in use. Close other OAuth flows and try again.`));
      } else {
        reject(err);
      }
    });
...
/**
 * Run Anthropic OAuth setup flow inline (popup + local callback)
 * Equivalent to Codex OAuth flow but using Anthropic PKCE tokens.
 */
async function runAnthropicOAuthFlow() {
  console.log('');
  const callbackBaseUrl = `http://127.0.0.1:${ANTHROPIC_OAUTH_CALLBACK_PORT}${ANTHROPIC_OAUTH_CALLBACK_PATH}`;

  const { authUrl: baseAuthUrl, verifier: pkceVerifier } = getAuthorizationUrl(callbackBaseUrl);
  const authUrl = new URL(baseAuthUrl);
  const expectedState = authUrl.searchParams.get('state');

  if (!expectedState) {
    throw new Error('OAuth flow initialization failed: missing state/verifier');
  }

  const codePromise = startAnthropicOAuthCallbackServer(expectedState);
  openBrowser(authUrl.toString());

  info('Opened browser for Anthropic authentication...');
  info(`If your browser does not open, open manually: ${authUrl.toString()}`);
  info(`Waiting for callback on ${callbackBaseUrl}`);

  const code = await codePromise;

  const tokenData = await exchangeCodeForTokens(code, expectedState, pkceVerifier, callbackBaseUrl);
  await storeToken(
 ...
 * Step 1: AI Providers
 */
async function stepProviders(config, projectRoot) {
  ...
  for (const provider of providers) {
    ...
    if (provider === 'openai') {
      ...
      if (authMethod === 'oauth') {
        ...
        const { loginWithCodexOAuth, saveCredentials } = require('./oauth-codex.cjs');
    ...
    if (provider === 'anthropic') {
      ...
      if (authMethod === 'oauth') {
        ...
        const spin = spinner('Running Anthropic OAuth flow...');
        spin.start();

        try {
          await runAnthropicOAuthFlow();
          spin.stop('success');
          ...
          info('You can try again from settings or use an API key.');
          if (await confirm('Continue setup without Anthropic OAuth?', true)) {
            config.providers.anthropic = { enabled: false, oauth: true, api_key: '' };
          } else {
            throw err;
          }
        }
...
async function stepServer(config) {
  printStep(3, 5, 'Server Configuration');
...
async function stepService(config) {
  printStep(4, 5, 'Install as Service');
  ...
  if (status.installed) {
    ...
    if (await confirm('Reinstall service?', false)) {
      const spin = spinner('Reinstalling service...');
      spin.start();
      ...
      try {
        ...
        if (result.success) {
          spin.stop('success');
          success('Service reinstalled and started');
        } else {
          spin.stop('error');
    ...
    if (await confirm('? Install as system service?', true)) {
      const spin = spinner('Installing service...');
      spin.start();
      ...
      try {
        ...
        spin.stop('error');
        error(`Installation failed: ${err.message}`);
        info('You can install manually later: evobrew daemon install');
      }
    } else {
...
async function setupWizard(projectRoot) {
  ...
  let currentStep = 0;
  const stepNames = ['Initialization', 'AI Providers', 'OpenClaw Integration', 'Server Configuration', 'Service Installation', 'Verification'];
  ...
  try {
    ...
    // Save after each step for incremental progress
    ...
    error(`Setup failed at Step ${currentStep} (${stepNames[currentStep]}): ${err.message}`);
