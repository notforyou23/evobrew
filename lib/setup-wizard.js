 ...
 * Step 1: AI Providers
 */
async function testOllamaCloud(apiKey) {
  return new Promise((resolve) => {
    // Use /v1/models — lightweight, no tokens spent, proves key is valid
    const options = {
      hostname: 'ollama.com',
      port: 443,
      path: '/v1/models',
      ...
      res.on('end', () => {
        if (res.statusCode === 200) {
          try {
            ...
            resolve({ valid: true, modelCount: count });
          } catch {
            resolve({ valid: true, modelCount: 0 });
          }
...
async function stepProviders(config, projectRoot, providerFilter = null) {
  ...
  const providerOptions = [
    ...
    { value: 'ollama-cloud', label: 'Ollama Cloud', hint: 'Nemotron, Qwen 397B, DeepSeek 671B, Kimi 1T...' },
  ...
  for (const provider of providers) {
    ...
    if (provider === 'ollama-cloud') {
      info('Get your API key at: https://ollama.com/settings/api-keys');
      const key = await questionSecret('? Ollama Cloud API Key');

      if (key) {
        const spin = spinner('Testing Ollama Cloud connection...');
        spin.start();

        const result = await testOllamaCloud(key);

        if (result.valid) {
          spin.stop('success');
          const modelMsg = result.modelCount > 0 ? ` (${result.modelCount} models available)` : '';
          config.providers['ollama-cloud'] = { enabled: true, api_key: key };
          success(`Ollama Cloud configured${modelMsg}`);
        } else {
          spin.stop('error');
          error(`Ollama Cloud test failed: ${result.error}`);

          if (await confirm('Save anyway?', false)) {
            config.providers['ollama-cloud'] = { enabled: true, api_key: key };
            warning('Ollama Cloud saved (unverified)');
          }
        }
      }
    }
    ...
    if (provider === 'local-agents') {
      ...
      let addMore = true;
      while (addMore) {
        const name = await question('? Agent name (e.g., COZ, Edison)', '');
        if (!name) break;

        const id = name.toLowerCase().replace(/[^a-z0-9-]/g, '-');
        const url = await question('? Agent URL', 'http://localhost:');
        const endpoint = await question('? Chat endpoint', '/api/chat');
        ...
        if (testOk) {
          success('Agent reachable');
        } else {
          warning('Agent not reachable (saving config anyway — start the agent later)');
        }
        ...
        success(`Saved: ${name} (local:${id})\n`);

        const more = await question('? Add another agent? (y/n)', 'n');
...
async function getConfigStatus(config = null) {
  ...
  const openaiApiConfigured = !!(providers.openai?.enabled && providers.openai?.api_key);
  const openaiApiStatus = openaiApiConfigured ? 'API key configured' : 'Not configured';
  ...
  const ollamaCloudConfigured = !!(providers['ollama-cloud']?.enabled && providers['ollama-cloud']?.api_key);
  const ollamaCloudStatus = ollamaCloudConfigured ? 'API key configured' : 'Not configured';
  ...
  return {
    ...
    ollamaCloud: {
      configured: ollamaCloudConfigured,
...
async function setupWizard(projectRoot, options = {}) {
  ...
  if (Array.isArray(options.only) && options.only.length > 0) {
    ...
    if (chosen === '1') {
      ...
      if (!configStatus.openaiApi.configured && !configStatus.openaiCodex.configured) missing.push('openai');
      if (!configStatus.xai.configured) missing.push('xai');
      if (!configStatus.ollamaCloud.configured) missing.push('ollama-cloud');
      ...
      const reconfigure = await multiSelect('Select providers to reconfigure:', [
        ...
        { value: 'ollama-cloud', label: 'Ollama Cloud', hint: 'Cloud API key' },
