...
/**
 * Check if COSMO IDE has working OAuth via its API
 * Does NOT copy database files - tokens are encrypted with different keys
 */
async function checkCosmoIDEOAuth() {
  return new Promise((resolve) => {
    // Check COSMO IDE API on default port
    const req = http.get('http://localhost:4410/api/oauth/anthropic/status', (res) => {
      let data = '';
      res.on('data', chunk => data += chunk);
      res.on('end', () => {
        try {
          if (res.statusCode === 200) {
            ...
            if (status.authenticated) {
              resolve({ hasOAuth: true, email: status.email });
              return;
            }
          }
        } catch {
          // Parse error, no OAuth
        }
        resolve({ hasOAuth: false });
      });
    });
    
    req.on('error', () => resolve({ hasOAuth: false }));
    req.setTimeout(3000, () => {
      req.destroy();
      resolve({ hasOAuth: false });
    });
 ...
 * OAuth must be done post-setup via web UI - tokens are encrypted and can't be shared
 */
async function runOAuthImport(projectRoot) {
  console.log('\n' + colors.dim + 'Checking OAuth status...' + colors.reset);
  
  // Check if COSMO IDE has OAuth configured (informational only)
  const cosmoStatus = await checkCosmoIDEOAuth();
  if (cosmoStatus.hasOAuth) {
    info(`COSMO IDE has OAuth configured${cosmoStatus.email ? ` (${cosmoStatus.email})` : ''}`);
    info('Note: OAuth tokens cannot be shared between installations.');
  }
  
  // Return guidance for post-setup OAuth
  return { 
    success: false, 
    error: 'OAuth must be configured after setup via the web UI.',
    guidance: true
  };
}
  ...
  for (const provider of providers) {
    ...
    if (provider === 'anthropic') {
      ...
      if (authMethod === 'oauth') {
        ...
        info('OAuth must be configured via the web UI after setup completes.');
        console.log('');
        
        // Check if COSMO IDE has OAuth (informational)
        await runOAuthImport(projectRoot);
        
        // Mark as OAuth-enabled (will authenticate via web UI)
        config.providers.anthropic = { enabled: true, oauth: true, api_key: '' };
        
        console.log('');
        success('Anthropic OAuth selected');
        info('After setup, authenticate at: http://localhost:' + 
          (config.server?.http_port || 3405) + '/api/oauth/anthropic/start');
      } else {
